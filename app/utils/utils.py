...
def task_dir(sub_dir:str) -> str:
    ...
    Returns:
        str: 任务目录的绝对路径
    """
    rootPath = get_root_dir()
    d = os.path.join(rootPath, "tasks")
    if sub_dir:
        d = os.path.join(d, sub_dir)
...
def run_in_background(func, *args,**kwargs):
    def run():
        try:
            ...
            logger.error(f"Error running function in background: {e}")
...
def str_contains_punctuation(word):
    return any(p in word for p in const.PUNCTUATIONS)
